...
(function ($) {
    ...
    $(document).on('ready', function () {
        $.getJSON("data.json", function (json) {
            ...
            addSkills(json.skills);
            addCoading(json.coding);
        });
    });
...
`
    ...
    function addSkills(skills) {
        var j;
        for (j = 0; j < skills.length; j = j + 2) {
            var skill1 = skills[j];
            var skill2 = skills[j + 1];

            var html = `
<div class="clearfix">
<h4>${skill1}</h4>
<div class="skill-value"><h4>${skill2}</h4></div>
</div>
`
        var txt3 = document.createElement("p");  // Create with DOM
        txt3.innerHTML = html
        document.getElementById("skills").append(txt3);
        }
    }

    function addCoading(coding) {
        var j;
        for (j = 0; j < coding.length; j = j +2 ) {
            var skill1 = coding[j];
            var skill2 = coding[j + 1];
...
`
        var txt3 = document.createElement("p");  // Create with DOM
        txt3.innerHTML = html
        document.getElementById("coding").append(txt3);
        }
    }
